...
copy_data(struct archive *ar, struct archive *aw)
{
    ...
    for (;;) {
        ...
        if (archive_write_data_block(aw, buff, size, offset) < ARCHIVE_OK) {
            INFO("Archive: %s", archive_error_string(aw));
            return (r);
        }
    }
}
...
-(ArchiveFile*)initWithFile:(NSString*)filename
{
    if (![[NSFileManager defaultManager] fileExistsAtPath:filename]) {
        INFO("Archive: File \"%s\" does not exist", filename.UTF8String);
        return nil;
    }
    self = [self init];
    _files = nil;
...
-(BOOL)extractFileNum:(int)fileNum toFd:(int)fd
{
    ...
    if (fd < 0) {
        INFO("Archive: invalid fd");
        return NO;
    }
    
    if (fileNum < 1) {
        INFO("Archive: invalid fileNum");
        return NO;
    }
    ...
    if (archive_read_open_fd(a, _fd, 16384) != ARCHIVE_OK) {
        INFO("Archive: unable to archive_read_open_fd: %s", archive_error_string(a));
        close(fd);
        return result;
    }
    ...
    if (rv == ARCHIVE_EOF) {
        INFO("Archive: no file %d", fileNum);
        goto out;
    }
    
    if (rv < ARCHIVE_OK) {
        INFO("Archive: %s", archive_error_string(a));
        if (rv < ARCHIVE_WARN)
            goto out;
    }
...
-(BOOL)extract:(NSString*)file toFd:(int)fd
{
    ...
    if (rv == ARCHIVE_EOF) {
        INFO("Archive: no such file \"%s\"", file.UTF8String);
        goto out;
    }
    ...
    if (entry && (strcmp(archive_entry_pathname(entry), file.UTF8String) != 0) ) {
        INFO("Archive: Unable to find entry for %s", file.UTF8String);
        goto out;
    }
...
-(BOOL)extract:(NSString*)file toPath:(NSString*)path
{
    ...
    int fd = dup(_fd);
    if (fd == -1) {
        INFO("Archive: unable to dupe fd");
        return NO;
    }
    ...
    if (rv < ARCHIVE_OK) {
        INFO("Archive: Unable to write header for %s: %s", path.UTF8String, archive_error_string(ext));
        if (rv < ARCHIVE_WARN)
            goto out;
    }
    if (archive_entry_size(entry) > 0) {
        rv = copy_data(a, ext);
        if (rv < ARCHIVE_OK) {
            INFO("Archive: Error copying data for %s: %s", path.UTF8String, archive_error_string(ext));
            if (rv < ARCHIVE_WARN)
                goto out;
        }
    }
    ...
    if (rv < ARCHIVE_OK) {
        INFO("Archive: %s", archive_error_string(ext));
        if (rv < ARCHIVE_WARN)
            goto out;
    }
    result = YES;
...
-(BOOL)extractToPath:(NSString*)path withFlags:(int)flags overWriteDirectories:(BOOL)overwrite_dirs
{
    ...
    if (![fm changeCurrentDirectoryPath:path]) {
        INFO("Archive: unable to change cwd to %s", path.UTF8String);
        goto out;
    }
    while ((rv = archive_read_next_header(a, &entry)) == ARCHIVE_OK) {
        if (rv < ARCHIVE_OK) {
